...
These lanes remain `experimental`, not `supported`, because the full driver/version matrix, prepared cursors, cancellation, reconnect storms, multi-identity authorization, and long-duration workload gates have not passed. The Go driver now passes binary prepared inserts, queries, signed/null/decimal/Unicode/temporal/blob values, 5 MiB long data, and prepared work inside a rollback transaction on both server families. Pooled client TLS, credential separation, downgrade refusal, and certificate rotation also have Go-driver coverage. Transparent mode remains the byte-preserving one-client/one-upstream compatibility fallback.
