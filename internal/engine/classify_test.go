...
func TestClassifySQL(t *testing.T) {
	for _, test := range []struct {
		...
		{"SHOW COUNT(*) WARNINGS", StatementWarningCount},
		...
		{"CALL dangerous_proc()", StatementUnsupported},
		{"LOAD DATA LOCAL INFILE 'x' INTO TABLE t", StatementUnsupported},
		{"LOCK TABLES t WRITE", StatementUnsupported},
		{"UNLOCK TABLES", StatementUnsupported},
		{"PREPARE s FROM 'SELECT 1'", StatementUnsupported},
		{"EXECUTE s", StatementUnsupported},
		{"DEALLOCATE PREPARE s", StatementUnsupported},
		{"DO SLEEP(1)", StatementUnsupported},
		{"HANDLER t OPEN", StatementUnsupported},
		{"OPTIMIZE TABLE t", StatementUnsupported},
		{"CHECK TABLE t", StatementUnsupported},
		{"REPAIR TABLE t", StatementUnsupported},
		{"RESET MASTER", StatementUnsupported},
		{"KILL 42", StatementUnsupported},
