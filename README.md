...
Pooled mode currently accepts `COM_QUERY`, `COM_PING`, `COM_INIT_DB`, `COM_QUIT`, and the common binary prepared-statement prepare/execute/long-data/reset/close flow. Prepared sessions pin one physical connection; cursors and fetch remain refused. It supports conservative single-statement text and prepared reads/writes, `BEGIN`/`START TRANSACTION`, `COMMIT`, `ROLLBACK`, savepoints, `SET AUTOCOMMIT`, `SET NAMES`, and `USE` for the configured database. It refuses multi-statements, unknown state-changing `SET` statements, stored procedure calls, temporary objects, locks, local file loading, and other unproven behavior. A refused command is not silently sent through a different session.
